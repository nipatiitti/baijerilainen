...
Surrogate model for BSFC response surface - fits per RPM bin
"""
...
class BSFCGaussianProcess:
    """
    Gaussian Process surrogate model for BSFC prediction within a single RPM bin.
    Models BSFC = f(Lambda, Timing) for a specific RPM range.
    """

    def __init__(self, noise_level: float = 0.1, n_restarts: int = 5):
        """
        ...
        # 2D kernel for [lambda, timing]
        kernel = ConstantKernel(1.0) * Matern(length_scale=[1.0, 1.0], nu=2.5)
    ...
    def fit(self, X: np.ndarray, y: np.ndarray) -> "BSFCGaussianProcess":
        ...
        Args:
            X: Training inputs, shape (n_samples, 2) for [lambda, timing]
    ...
    def predict(self, X: np.ndarray, return_std: bool = True) -> Tuple[np.ndarray, Optional[np.ndarray]]:
        ...
        if return_std:
            mean_scaled, std_scaled = self.gp.predict(
                X_scaled, return_std=True)

            # Inverse transform predictions
            mean = self.y_scaler.inverse_transform(
                mean_scaled.reshape(-1, 1)).ravel()
    ...
    def predict_grid(
        ...
        """
        Predict BSFC over a 2D grid of lambda and timing.
        ...
        X_grid = np.column_stack([L.ravel(), T.ravel()])
