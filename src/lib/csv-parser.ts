 ...
 */
export function createSimplifiedCSV(parsed: ParsedMotecCSV, selectedColumns: string[]): string {
  // Find indices of selected columns
  const indices = selectedColumns.map((name) => parsed.headers.indexOf(name)).filter((i) => i !== -1);
 ...
 */
export function processMotecCSV(content: string, selectedColumns: string[]): string {
  // Parse the full CSV
